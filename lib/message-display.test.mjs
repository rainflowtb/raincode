...
test("re-extracts prompt text and base64 images from a persisted user message", async () => {
  const { getUserMessageTextAndImages } = await loadSubject();

  assert.deepEqual(
    getUserMessageTextAndImages({ role: "user", content: "hello" }),
    { text: "hello", images: [] },
  );
  assert.deepEqual(
    getUserMessageTextAndImages({
      role: "user",
      content: [
    ...
    { text: "look at this", images: [{ data: "QUJD", mimeType: "image/png" }] },
  );
  assert.deepEqual(
    getUserMessageTextAndImages({
      role: "user",
    ...
    { text: "", images: [{ data: "QUJD", mimeType: "image/png" }] },
  );
});
