...
const {
  ...
  replaceMemoryFact,
  retainMemoryFact,
  selectMemoryInjectFacts,
} = pm;
const { buildQueryMemoryContext } = await import("./memory-context.ts");
...
describe("secret guard patterns", () => {
  it("rejects common token formats, not just 'key: value' phrasing", () => {
    const cwd = fresh();
    const settings = tightSettings();
    const secrets = [
      "github token is ghp_abcdefghij1234567890ABCD",
      "fine-grained github_pat_11ABCDEFGH0ijklmnopqrstuvwxyz0123456789_ABCDEFG",
      "oauth gho_abcdefghij1234567890ABCD",
    ...
    for (const text of secrets) {
      assert.throws(
        () => retainMemoryFact(cwd, text, { settings }),
        /Refusing to store possible secrets/,
        `should reject: ${text.slice(0, 40)}`,
      );
    }
    assert.equal(listMemoryFacts(cwd).length, 0);
  });

  it("still allows facts that merely talk about secrets", () => {
    const cwd = fresh();
    const settings = tightSettings();
    retainMemoryFact(cwd, "never commit credentials; rotate them quarterly via the vault", { settings });
    assert.equal(listMemoryFacts(cwd).length, 1);
  });
});

describe("selectMemoryInjectFacts", () => {
  it("is the single selection owner: buildMemoryInjectBlock renders exactly its picks", () => {
    const cwd = fresh();
    const settings = tightSettings({ enabled: true, autoInject: true, maxInjectChars: 12000 });
    retainMemoryFact(cwd, "fact one for injection", { settings });
    retainMemoryFact(cwd, "fact two for injection", { settings });

    const picked = selectMemoryInjectFacts(cwd, settings);
    assert.equal(picked.length, 2);
    const block = buildMemoryInjectBlock(cwd, settings);
    for (const fact of picked) assert.ok(block.includes(`- ${fact.text}`));
  });

  it("honors maxInjectChars and returns [] when autoInject is off", () => {
    const cwd = fresh();
    const settings = tightSettings({ enabled: true, autoInject: true, maxInjectChars: 12000 });
    retainMemoryFact(cwd, "a fact long enough to matter " + "x".repeat(200), { settings });
    // Header alone (~130 chars) + one ~232-char line exceeds a 200-char inject budget.
    const tiny = tightSettings({ enabled: true, autoInject: true, maxInjectChars: 200 });
    assert.deepEqual(selectMemoryInjectFacts(cwd, tiny), []);
    assert.deepEqual(
      selectMemoryInjectFacts(cwd, tightSettings({ enabled: true, autoInject: false })),
      [],
    );
  });
});

describe("buildQueryMemoryContext frozen dedupe", () => {
  /** Enable autoInject through the on-disk web settings of a throwaway agent dir. */
  function freshWithAutoInject() {
    const agentDir = mkdtempSync(join(tmpdir(), "pi-mem-test-agent-"));
    process.env.PI_CODING_AGENT_DIR = agentDir;
    writeFileSync(
      join(agentDir, "raincode.json"),
      JSON.stringify({ projectMemory: { enabled: true, autoInject: true } }),
    );
    return mkdtempSync(join(tmpdir(), "pi-mem-test-cwd-"));
  }

  it("skips only facts in the frozen snapshot, not the live store", () => {
    const cwd = freshWithAutoInject();
    const settings = tightSettings({ enabled: true, autoInject: true, maxInjectChars: 12000 });
    retainMemoryFact(cwd, "alphawidget builds with vite", { settings });
    retainMemoryFact(cwd, "betagadget deploys via ansible", { settings });

    // Snapshot froze only the alpha fact (session start); beta entered the
    // live top-K later and must still be recalled, alpha must not repeat.
    const block = buildQueryMemoryContext(cwd, "alphawidget betagadget", ["alphawidget builds with vite"]);
    assert.ok(block);
    assert.match(block, /- betagadget deploys via ansible/);
    assert.doesNotMatch(block, /alphawidget builds with vite/);

    // Nothing left to say when every match is already in the snapshot.
    assert.equal(buildQueryMemoryContext(cwd, "alphawidget", ["alphawidget builds with vite"]), null);
  });

  it("returns null when autoInject is off in web settings", () => {
    const cwd = fresh(); // no raincode.json → defaults: autoInject off
    const settings = tightSettings();
    retainMemoryFact(cwd, "gammagizmo uses pnpm", { settings });
    assert.equal(buildQueryMemoryContext(cwd, "gammagizmo", []), null);
  });
});
