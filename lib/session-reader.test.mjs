...
const {
  ...
  childTasksDir,
  resolveSessionPathInTasksDir,
} = await jiti.import("./session-reader.ts");
// Parsed entries + UI context live in session-entries.ts (the SDK-backed half).
const { buildSessionContext, buildUsageMessages } = await jiti.import("./session-entries.ts");
...
test("keeps the full branch history with the compaction card inline", () => {
  const entries = [
  ...
  assert.deepEqual(
    context.messages.map((message) => [message.role, message.customType ?? null]),
    [
      ["user", null],
      ["assistant", null],
      ["user", null],
  ...
  assert.equal(context.messages[3].content, "old exchange summary");
});

test("keeps every compaction card inline on the active path", () => {
  const entries = [
  ...
  assert.equal(context.messages[3].role, "custom");
  assert.equal(context.messages[3].content, "first summary");
  assert.equal(context.messages[6].role, "custom");
  assert.equal(context.messages[6].content, "latest summary");
  assert.equal(context.messages.length, context.entryIds.length);

  // Usage estimation still uses the compacted model context only.
  const usage = buildUsageMessages(entries);
  assert.deepEqual(
    usage.map((message) => [message.role, message.customType ?? null]),
    [
      ["custom", "compaction"],
      ["assistant", null],
      ["user", null],
  ...
  assert.equal(usage[0].content, "latest summary");
});
...
test("keeps forward and reverse session path caches in sync", async () => {
  ...
  assert.equal(globalThis.__raincodeSessionPathCache?.has(sessionId), false);
  assert.equal(globalThis.__raincodePathToSessionIdCache?.has(normalize(filePath)), false);
});

test("resolves a child session under parent/tasks without listing it globally", async () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-child-tasks-"));
  const parentFile = join(dir, "20260101_parent.jsonl");
  const childId = "01234567-89ab-4def-8123-456789abcdef";
  const tasks = childTasksDir(parentFile);
  mkdirSync(tasks, { recursive: true });
  const childFile = join(tasks, `20260101_${childId}.jsonl`);
  ...
  try {
    assert.equal(await resolveSessionPathInTasksDir(parentFile, childId), childFile);
    assert.equal(await resolveSessionPathInTasksDir(parentFile, "missing"), null);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
