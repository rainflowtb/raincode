...
test("configures HTTP_PROXY, HTTPS_PROXY, and NO_PROXY for global fetch", async (t) => {
  ...
  // A proxy receives http:// traffic as absolute-form plain requests and
  // https:// traffic as CONNECT tunnels — record both shapes.
  const connectTargets = [];
  const plainRequests = [];
  const proxy = createServer((req, res) => {
    plainRequests.push(`${req.method} ${req.url}`);
  ...
  assert.deepEqual(plainRequests, ["GET http://target.invalid/through-http-proxy"]);
  ...
  const proxiedRequestCount = connectTargets.length + plainRequests.length;
  await assert.rejects(fetch("http://bypass.invalid:9/no-proxy", {
    signal: AbortSignal.timeout(2_000),
  }));
  assert.equal(connectTargets.length + plainRequests.length, proxiedRequestCount);
});
