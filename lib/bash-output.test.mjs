...
test("accepts only pi bash logs directly inside the configured temp directory", async () => {
  ...
  // resolveBashOutputPath returns resolve(filePath), so the expectations have to
  // be native paths — a hard-coded "/tmp/…" only matches on POSIX.
  const tempRoot = resolve(join(tmpdir(), "pi-web-output-tests"));
  const logPath = join(tempRoot, "pi-bash-ab12.log");

  assert.equal(resolveBashOutputPath(logPath, tempRoot), logPath);
