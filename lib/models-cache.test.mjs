...
test("attaches model runtime errors only when present", () => {
  const data = modelsData("ok");
  assert.deepEqual(withModelRuntimeError(data, undefined), data);
  assert.deepEqual(withModelRuntimeError(data, "bad models.json"), {
    ...data,
    modelError: "bad models.json",
  });
});
