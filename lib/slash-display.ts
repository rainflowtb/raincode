 ...
 *
 * Requires the opening envelope, matching base-directory reference, final
 * closing tag, and an optional two-newline args suffix — so ordinary text that
 * merely starts with a skill-looking tag is not collapsed. The greedy body
 * capture makes the final closing tag win when a skill body contains an example
 * `</skill>` tag.
 */
