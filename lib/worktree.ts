...
declare global {
  var __raincodeProjectCache: Map<string, { info: ProjectInfo; expiresAt: number }> | undefined;
}

const PROJECT_CACHE_TTL_MS = 60_000;

function getProjectCache(): Map<string, { info: ProjectInfo; expiresAt: number }> {
  if (!globalThis.__raincodeProjectCache) globalThis.__raincodeProjectCache = new Map();
  return globalThis.__raincodeProjectCache;
}

export function invalidateProjectCache(): void {
  globalThis.__raincodeProjectCache?.clear();
}
