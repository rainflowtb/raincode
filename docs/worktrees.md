# Worktrees in RainCode

RainCode can show all Git worktrees for one project in the sidebar. Use this when you want to keep separate checkouts for different branches, while keeping the project's sessions grouped together.
...
Use the worktree switcher to choose which checkout RainCode should use for new work in that project.
...
RainCode creates the checkout at:
...
If the branch already exists, RainCode adds a worktree for that branch. If it does not exist, RainCode creates the branch from the current `HEAD`.
...
- The Git branch.
- RainCode session history.
- The main checkout.

If the worktree has uncommitted or untracked files, Git refuses the removal. RainCode then offers a force remove action. Force removal discards the uncommitted files in that checkout, so use it only when you no longer need those changes.

## Sessions and Worktrees

RainCode groups sessions by project root, so sessions from the main checkout and linked worktrees appear together.
...
Git can keep prunable worktree records after a checkout disappears. RainCode filters those out of the switcher.
