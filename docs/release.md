...
- npm package: `@ct-jyjntc/pi-web`
- GitHub Release: `ct-jyjntc/pi-web`
...
- npm is authenticated as an account that can publish `@ct-jyjntc/pi-web`.
...
- If `npm view @ct-jyjntc/pi-web version` briefly shows the previous version, check the exact version instead:

```bash
npm view @ct-jyjntc/pi-web@<version> version --registry https://registry.npmjs.org/
npm view @ct-jyjntc/pi-web versions --json --registry https://registry.npmjs.org/
```
...
gh release view v<version> --repo ct-jyjntc/pi-web
```
...
- 发布 npm 包 `@ct-jyjntc/pi-web@<version>`。
...
- Published npm package `@ct-jyjntc/pi-web@<version>`.
```
...
```bash
gh release edit v<version> --repo ct-jyjntc/pi-web --notes-file - <<'EOF'
## 中文
