# RainCode 里的 Worktree

RainCode 会把同一个 Git 项目的 main checkout 和 linked worktree 放在同一个项目下。你可以用它在不同分支之间切换工作目录，同时保留统一的会话列表。
...
worktree 切换器决定 RainCode 接下来使用哪个 checkout。
...
如果这个 branch 已存在，RainCode 会为它添加 worktree。如果 branch 不存在，RainCode 会从当前 `HEAD` 创建这个 branch。
...
如果 worktree 里有未提交或未跟踪文件，Git 会拒绝删除。RainCode 会再显示 force remove。force remove 会丢弃这个 checkout 里的未提交文件，只在确定不需要这些改动时使用。

## 会话和 Worktree 的关系

RainCode 按 project root 分组会话，所以 main checkout 和 linked worktree 里的会话会显示在一起。
...
Git 有时会保留 prunable worktree 记录。RainCode 会过滤这些记录，不在切换器里显示。
