...
declare global {
  var __raincodeFileIndexCache: Map<string, CacheEntry> | undefined;
}

function getIndexCache(): Map<string, CacheEntry> {
  if (!globalThis.__raincodeFileIndexCache) globalThis.__raincodeFileIndexCache = new Map();
  return globalThis.__raincodeFileIndexCache;
}
...
async function listWithWalk(cwd: string): Promise<FileListing> {
  ...
  for (let head = 0; head < queue.length; head++) {
    ...
    for (const d of dirents) {
      if (isIgnoredDirentName(d.name)) continue;
